    ...
    <Pressable style={({ pressed }) => [styles.card, pressed && styles.cardPressed]} onPress={onPress}>
      <View style={styles.topRow}>
        ...
        <View style={styles.textWrap}>
          ...
          <Text style={styles.preview} numberOfLines={3}>{block.summary}</Text>
        </View>
      </View>
      {block.citations.length ? <Text style={styles.citations}>{block.citations.join(' • ')}</Text> : null}
    </Pressable>
  );
}
...
const styles = StyleSheet.create({
  ...
  confidence: { fontSize: 11, lineHeight: 15, color: palette.success, fontWeight: '700', textTransform: 'capitalize' },
